	...
	$name_notification  = 'Submit Assignment file name'." ".'<b>'.$name.'</b>'; 
        ...
        if (!file_exists($newname)) {
            ...
            if ((move_uploaded_file($_FILES['uploaded_file']['tmp_name'], $newname))) {
                ...
                $qry2 = ("INSERT INTO student_assignment (fdesc,floc,assignment_fdatein,fname,assignment_id,student_id) VALUES ('$filedesc','$newname',NOW(),'$name','$assignment_id','$session_id')")or die(mysqli_error($conn));
				mysqli_query($conn,"insert into teacher_notification (teacher_class_id,notification,date_of_notification,link,student_id,assignment_id) value('$get_id','$name_notification',NOW(),'view_submit_assignment.php','$session_id','$assignment_id')")or die(mysqli_error($conn));
